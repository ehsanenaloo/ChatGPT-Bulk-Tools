  ...
  const ConversationHandler = {
    ...
    async performOperation(operation, selectedConversations, buttonId) {
      ...
      if (totalCount === 0) {
        ...
        ChromeUtils.sendComplete(buttonId, { processed: 0, total: 0, action: operation });
        return;
      }

      console.log(`Selected conversations for ${operation}:`, totalCount);

      if (window.GlobalState && GlobalState.clearStopRequest) {
        GlobalState.clearStopRequest();
      }
      ...
      for (let i = 0; i < totalCount; i++) {
        if (window.GlobalState && GlobalState.isStopRequested && GlobalState.isStopRequested()) {
          console.log(`${operation} stopped by user after ${processedCount} processed`);
          break;
        }

        let currentTitle = '';
        try {
          const checkbox = selectedConversations[i];
          const conversationElement = checkbox?.parentElement || checkbox;
          currentTitle = (window.DOMHandler && DOMHandler.getConversationTitle)
            ? DOMHandler.getConversationTitle(conversationElement)
            : '';
          const result = await this.processConversation(operation, checkbox);
          if (result) {
            processedCount++;
          } else {
            skippedCount++;
      ...
      ChromeUtils.sendComplete(buttonId, { processed: processedCount, total: totalCount, action: operation });
    },
